...
def load_crash_data(file):
    ...
    if "I-25 Segment 5 Accident History" in str(df.iloc[0, 0]):
        ...
        if "Date" not in df.columns:
            st.error("Missing 'Date' column after renaming. Please check the format of your Segment 5 file.")
            return pd.DataFrame()

        if "Severity" not in df.columns:
            st.error("Missing 'Severity' column after renaming. Please check the format of your Segment 5 file.")
            return pd.DataFrame()
